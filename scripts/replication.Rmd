...
sapply(pse_all, function(x) sum(is.na(x)))
```

```{r data cleaning}
names(BYF4STU) <- str_to_lower(names(BYF4STU)) #lowercase variable names

data <- BYF4STU %>% dplyr::select(c(stu_id,f4efsect, f4elsect, f4hhdg, bys12, race, byses, f22xrstd, f22xmstd, f1hsprog,bys82o, f1s41bc, f2s30bc, f1d63, marstat, bypsepln, f2s76))
...
data <- data %>% mutate(acapro = ifelse(f1hsprog==2,1,0),
...
data <- merge(data,pse_all, by="stu_id") 
